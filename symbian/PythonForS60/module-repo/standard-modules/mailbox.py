...
import StringIO
try:
    if sys.platform == 'os2emx' or sys.platform == 'symbian_s60':
        # OS/2 EMX and Symbian fcntl() not adequate
        raise ImportError
    import fcntl
except ImportError:
    fcntl = None
...
class Maildir(Mailbox):
    ...
    def clean(self):
        ...
        for entry in os.listdir(os.path.join(self._path, 'tmp')):
            ...
            # Access time and modification time are the same on Symbian. We
            # cannot set access time but only modification time.
            if sys.platform == 'symbian_s60':
                time_diff = now - os.path.getmtime(path)
            else:
                time_diff = now - os.path.getatime(path)
            if time_diff > 129600:   # 60 * 60 * 36
                os.remove(path)
